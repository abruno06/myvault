...
func connectVaulwithYubikeyORI(ctx context.Context) (*vault.Client, error) {
	...
	caCert, err := os.ReadFile("./cert/server.pem") // You can change this to the path of your CA certificate
	if err != nil {
		log.Fatal(err)
	}
	...
	//log.Printf("Client1: %v", client)
	//set the transport configuration
	httpclient := client.Configuration().HTTPClient
	transport := httpclient.Transport
	transport.(*http.Transport).TLSClientConfig = tlsConfig

	//log.Printf("Client2-Trans: %v", transport)
	//client.Configuration().HTTPClient = &http.Client{Transport: &transport}
	// Authenticate with the Vault server using ceritificate
	resp, err := client.Auth.CertLogin(ctx, schema.CertLoginRequest{Name: readCertificateName()})
	if err != nil {
		log.Fatal(err)
